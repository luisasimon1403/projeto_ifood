...
Restaurante restaurante; /* apenas 1 restaurante */
...
/* Usuário fixo p/ login (agora com espaço e endereço separado) */
char usuario_fixo_nome[40] = "Joao";
char usuario_fixo_email[50] = "joao@email.com";
char usuario_fixo_senha[20] = "1234";
char usuario_fixo_endereco[100] = "Rua das Flores, 123 - Centro, São Paulo/SP";
char usuario_fixo_telefone[20] = "11988887777";

/* flag: 1 = usuário fixo está logado, 0 = não */
int usuario_logado_fixo = 0;
...
void cadastro_final() {
    ...
    printf("Endereco: ");
    fgets(endereco_usuario, sizeof(endereco_usuario), stdin);
    endereco_usuario[strcspn(endereco_usuario, "\n")] = '\0';

    do {
        printf("Senha: ");
        scanf("%19s", senha_usuario);
        printf("Confirmar senha: ");
        scanf("%19s", confirma_senha);
...
void tela_login() {
    int escolha;
    char email_local[50], senha_local[20];
    ...
    do {
        ...
        if (escolha == 1) {
            ...
            /* caso: usuario criado pelo fluxo do programa */
...
/* =========================
   ...
   =========================*/
void mostrarInformacoesUsuario() {
    if (!usuario_cadastrado && !usuario_logado_fixo) {
    ...
    if (usuario_logado_fixo) {
        printf("Nome: %s\n", usuario_fixo_nome);
        printf("Email: %s\n", usuario_fixo_email);
        printf("Endereco: %s\n", usuario_fixo_endereco);
        printf("Telefone: %s\n", usuario_fixo_telefone);
...
void editarInformacoesUsuario() {
    ...
    if (usuario_logado_fixo) {
        ...
        if (buf[0] != '\n') {
            buf[strcspn(buf, "\n")] = '\0';
            strncpy(usuario_fixo_nome, buf, sizeof(usuario_fixo_nome)-1);
            usuario_fixo_nome[sizeof(usuario_fixo_nome)-1] = '\0';
        }
        ...
        if (buf[0] != '\n') {
            buf[strcspn(buf, "\n")] = '\0';
            strncpy(usuario_fixo_endereco, buf, sizeof(usuario_fixo_endereco)-1);
            usuario_fixo_endereco[sizeof(usuario_fixo_endereco)-1] = '\0';
        }
		...
		if (buf[0] != '\n') {
    		buf[strcspn(buf, "\n")] = '\0';
    		strncpy(usuario_fixo_telefone, buf, sizeof(usuario_fixo_telefone)-1);
    		usuario_fixo_telefone[sizeof(usuario_fixo_telefone)-1] = '\0';
		}
...
void editarContatoUsuario() {
    ...
    if (usuario_logado_fixo) {
        ...
        if (buf[0] != '\n') {
            buf[strcspn(buf, "\n")] = '\0';
            strncpy(usuario_fixo_email, buf, sizeof(usuario_fixo_email)-1);
            usuario_fixo_email[sizeof(usuario_fixo_email)-1] = '\0';
        }

        printf("Telefone atual: Nao informado\n");
        printf("Novo telefone (enter para manter): ");
        fgets(buf, sizeof(buf), stdin); /* não armazenamos telefone do fixo, apenas ignora ou poderia salvar em outra var */
        /* ignorado */
    } else {
        printf("Email atual: %s\n", email_usuario);
        printf("Novo email (enter para manter): ");
...
void editarCredenciaisUsuario() {
    ...
    if (usuario_logado_fixo) {
        if (strcmp(senhaAtual, usuario_fixo_senha) != 0) {
            printf("\nSenha incorreta!\n");
            system("pause");
            return;
        }
        ...
        strcpy(usuario_fixo_senha, novaSenha);
    } else {
...
void verCardapioUsuario() {
    ...
    /* === opção direta para acessar o carrinho === */
    printf("\nC - Ver carrinho");
...
/* ===========================
   CARRINHO / PAGAMENTO / STATUS
   =========================== */
void confirmarEnderecoAntesPagamento() {
    system("cls");
    printf("\n=== CONFIRMAR ENDERECO ===\n");

    char endereco_atual[200];

    if (usuario_logado_fixo == 1) {
        strcpy(endereco_atual, usuario_fixo_endereco);
    } else {
        strcpy(endereco_atual, endereco_usuario);
    }

    printf("Endereco atual cadastrado:\n%s\n", (*endereco_atual) ? endereco_atual : "Nao informado");

    int op;
    printf("\nO endereco esta correto?\n");
    ...
    if (op == 2) {
        char novo[200];
        printf("\nDigite o novo endereco completo:\n");
        fgets(novo, sizeof(novo), stdin);
        novo[strcspn(novo, "\n")] = '\0';

        if (usuario_logado_fixo == 1) {
            strncpy(usuario_fixo_endereco, novo, sizeof(usuario_fixo_endereco)-1);
            usuario_fixo_endereco[sizeof(usuario_fixo_endereco)-1] = '\0';
        } else {
            strncpy(endereco_usuario, novo, sizeof(endereco_usuario)-1);
            endereco_usuario[sizeof(endereco_usuario)-1] = '\0';
        }
...
void montarCarrinho() {
    ...
    do {
        ...
        if (opc == 1) {
            ...
            printf("\nTotal: R$ %.2f\n", total);
            confirmarEnderecoAntesPagamento();
